...
int showGameMode(SDL_Renderer *renderer, GameModeInfo modeInfo, Tetromino *current, Tetromino *next, SDL_Texture *blockTextures[])
{
    ...
    while (running)
    {
        ...
        // Textes
        if (bestScoreTex)
            SDL_RenderCopy(renderer, bestScoreTex, NULL, &bestScoreRect);
        if (statsTex)
            SDL_RenderCopy(renderer, statsTex, NULL, &statsLabelRect);
        if (scoreTex)
            SDL_RenderCopy(renderer, scoreTex, NULL, &scoreTextRect);
        if (linesTex)
            SDL_RenderCopy(renderer, linesTex, NULL, &linesTextRect);
